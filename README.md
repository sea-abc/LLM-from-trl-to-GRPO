...
>1. 官方源码的 PPO 示例（经简单修改）
>2. 使用 Pythia-1B 模型进行演示
>3. 包含完整的 PPO 训练流程

-  `ppo_trl_qwen.py`

>1. 基于 Qwen3-4B-Instruct 模型的 PPO 训练示例代码
>2. 支持 4-bit 量化 + LoRA 微调
>3. 包含训练后的自动推理测试功能

- `ppo_trl_qwen_new.py`

>1. ppo_trl_qwen.py 的更新版本
>2. 支持加载已经训练好的奖励模型
>3. 功能与 ppo_trl_qwen.py 大致相同

- `inference_qwen_ppo.py`

>1. PPO 训练后模型单独的推理脚本
>2. 支持交互式交流和批量推理
>3. 使用 4-bit 量化加载训练好的模型
...
- 为了单卡能够训练，且下载时间不要太长，考虑所有脚本均默认使用 Qwen3-4B-Instruct-2507 模型
